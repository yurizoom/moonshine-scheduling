...
class CronSchedule
{
    ...
    // The language table
    private bool|array $_lang = false;
     ...
     */
    private function dtFromParameters($time = false): array
    {
        if ($time === false) {
            $arrTime = getdate();
    ...
    private function dtAsString($arrDt): bool|string
    {
        if ($arrDt === false) {
            return false;
        }
     ...
     */
    final public function next($time = false): array|bool
    {
        // Convert parameters to array datetime
    ...
    final public function nextAsString($time = false): bool|string
    {
        return $this->dtAsString($this->next($time));
    }

    final public function nextAsTime($time = false): bool|int
    {
     ...
     */
    private function advanceItem($arrItems, $rangeMin, $rangeMax, &$current): bool
    {
        // Advance pointer
        $current++;
     ...
     */
    private function getEarliestItem($arrItems, $afterItem = false, $allowOverflow = true): bool|int|string|null
    {
     ...
     */
    final public function previous($time = false): array|bool
    {
        // Convert parameters to array datetime
        $arrDT = $this->dtFromParameters($time);
    ...
    final public function previousAsString($time = false): bool|string
    {
        return $this->dtAsString($this->previous($time));
    }

    final public function previousAsTime($time = false): bool|int
    {
     ...
     */
    private function recedeItem($arrItems, $rangeMin, $rangeMax, &$current): bool
    {
        // Recede pointer
        $current--;
     ...
     */
    private function getLatestItem($arrItems, $beforeItem = false, $allowOverflow = true): bool|int|string|null
    {
     ...
     * Result:
     */
    private function getClass($spec): string
    {
     ...
     * Result:
     */
    private function classIsSpecified($spec): bool
    {
        if (!$spec['elements'][0]['hasInterval']) {
            return true;
        }
     ...
     * Result:
     */
    private function classIsSingleFixed($spec): bool
    {
        return (count($spec['elements']) == 1) && (!$spec['elements'][0]['hasInterval']);
    }

    private function initLang($language = 'en'): void
    {
        switch ($language) {
            case 'en':
    ...
    private function natlangPad2($number): string
    {
     ...
     * Result:
     */
    private function natlangRange($spec, $entryFunction, $p1 = false): string
    {
        $arrIntervals = [];
     ...
     */
    final public function asNaturalLanguage(): string
    {
